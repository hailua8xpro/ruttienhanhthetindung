...
namespace Nop.Services.News
{
    ...
    public partial class NewsService : INewsService
    {
            ...
            IDbContext dbContext)
        {
            ...
            this._newsCategoryMappingRepository = newsCategoryMappingRepository;
        ...
        public virtual IPagedList<NewsItem> SearchNews(int newsCategoryId = 0, string keywords = null, int languageId = 0, int storeId = 0,
            int pageIndex = 0, int pageSize = int.MaxValue, bool showHidden = false)
        {
            var query = _newsItemRepository.Table;
            if (languageId > 0)
            ...
            {
                query = from n in query
                        join ncm in _newsCategoryMappingRepository.Table
                        on n.Id equals ncm.NewsId
                        select n;
            }
            if (!string.IsNullOrEmpty(keywords))
            {
                query = query.Where(n => n.Title.Contains(keywords));
            }
            var news = new PagedList<NewsItem>(query, pageIndex, pageSize);
            return news;
        }
          ...
          ProductSortingEnum orderBy = ProductSortingEnum.CreatedOn)
        {
            ...
            var news = _dbContext.EntityFromSql<NewsItem>("NewsLoadAllPaged",
                pCategoryIds,
                pStoreId,
                pKeywords,
                pSearchDescriptions,
