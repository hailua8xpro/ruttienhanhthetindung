...
namespace Nop.Web.Areas.Admin.Helpers
{
    ...
    public static class SelectListHelper
    {
        ...
        public static List<SelectListItem> GetNewsCategoryList(INewsCategoryService newsCategoryService, ICacheManager cacheManager, bool showHidden = false)
        {
            if (newsCategoryService == null)
                throw new ArgumentNullException(nameof(newsCategoryService));

            if (cacheManager == null)
                throw new ArgumentNullException(nameof(cacheManager));

            var cacheKey = string.Format(ModelCacheEventConsumer.NEWSCATEGORIES_LIST_KEY, showHidden);
            var listItems = cacheManager.Get(cacheKey, () =>
            {
                var categories = newsCategoryService.GetAllNewsCategories(showHidden: showHidden);
                return categories.Select(c => new SelectListItem
                {
                    Text = newsCategoryService.GetFormattedBreadCrumb(c, categories),
                    Value = c.Id.ToString()
                });
            });
        ...
        public static List<SelectListItem> GetServiceCategoryList(IServiceCategoryService serviceCategoryService, ICacheManager cacheManager, bool showHidden = false)
        {
            ...
            var cacheKey = string.Format(ModelCacheEventConsumer.SERVICECATEGORIES_LIST_KEY, showHidden);
            var listItems = cacheManager.Get(cacheKey, () =>
            {
                var categories = serviceCategoryService.GetAllServiceCategories(showHidden: showHidden);
                return categories.Select(c => new SelectListItem
                {
                    Text = serviceCategoryService.GetFormattedBreadCrumb(c, categories),
                    Value = c.Id.ToString()
                });
            });
