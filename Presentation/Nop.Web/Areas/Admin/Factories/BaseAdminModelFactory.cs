...
namespace Nop.Web.Areas.Admin.Factories
{
    ...
    public partial class BaseAdminModelFactory : IBaseAdminModelFactory
    {
        ...
        #region custom
        /// <summary>
        /// Get category type base on bannertype
        /// </summary>
        /// <param name="items"></param>
        /// <param name="withSpecialDefaultItem"></param>
        /// <param name="defaultItemText"></param>
        /// <param name="bannerType"></param>
        public virtual void PrepareBannerCategoryEntity(IList<SelectListItem> items, bool withSpecialDefaultItem = true, string defaultItemText = null, BannerType bannerType = BannerType.Home)
        {
            if (items == null)
                throw new ArgumentNullException(nameof(items));
            ...
            switch (bannerType)
            {
                ...
                case BannerType.ProductCategory:
                    PrepareCategories(items, withSpecialDefaultItem, defaultItemText);
                    return;
                default:
                    break;
            }
        ...
        public virtual void PrepareNewsCategories(IList<SelectListItem> items, bool withSpecialDefaultItem = true, string defaultItemText = null)
        {
            if (items == null)
                throw new ArgumentNullException(nameof(items));

            //prepare available categories
            var availableNewsCategoryItems = SelectListHelper.GetNewsCategoryList(_newsCategoryService, _cacheManager, true);
            foreach (var categoryItem in availableNewsCategoryItems)
            {
                items.Add(categoryItem);
            }
        ...
        public virtual void PrepareServiceCategories(IList<SelectListItem> items, bool withSpecialDefaultItem = true, string defaultItemText = null)
        {
            if (items == null)
                throw new ArgumentNullException(nameof(items));

            //prepare available categories
            var availableNewsCategoryItems = SelectListHelper.GetServiceCategoryList(_serviceCategoryService, _cacheManager, true);
            foreach (var categoryItem in availableNewsCategoryItems)
            {
                items.Add(categoryItem);
            }
