    ...
    public partial class NewsModelFactory : INewsModelFactory
    {
        ...
        public virtual NewsItemModel PrepareNewsItemModel(NewsItemModel model, NewsItem newsItem, bool excludeProperties = false)
        {
            ...
            if (newsItem != null)
            {
                ...
                {
                    model.SelectedNewsCategoryIds = _newsCategoryService.GetNewsCategoryMappingByNewsId(newsItem.Id, true)
            ...
            _baseAdminModelFactory.PrepareNewsCategories(model.AvailableNewsCategories, false);
            foreach (var categoryItem in model.AvailableNewsCategories)
            {
                categoryItem.Selected = int.TryParse(categoryItem.Value, out var categoryId)
                    && model.SelectedNewsCategoryIds.Contains(categoryId);
            }
            return model;
        }
